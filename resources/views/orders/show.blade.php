...
<section class="auth-card" aria-labelledby="order-title">
    ...
    @endif
    @if ($order->payment_method === 'online' && ! in_array($order->payment_status, ['paid', 'refunded', 'refund_pending'], true) && $order->status !== 'cancelled')<p class="auth-links"><a href="{{ route('payments.show', $order) }}">Retry payment</a></p>@endif
    ...
    <dl class="account-details"><div><dt>Shipping address</dt><dd>{{ $order->shipping_name }}<br>{{ $order->shipping_phone }}<br>{{ $order->shipping_address }}</dd></div><div><dt>Subtotal</dt><dd>Rs {{ number_format($order->subtotal, 2) }}</dd></div><div><dt>Shipping</dt><dd>{{ $order->shipping_charge > 0 ? 'Rs '.number_format($order->shipping_charge, 2) : 'FREE' }}</dd></div><div><dt>Estimated delivery</dt><dd>{{ $order->delivery_estimate }}</dd></div>@if ($order->shipment?->awb)<div><dt>Tracking</dt><dd>{{ strtoupper($order->shipment->provider) }} · {{ $order->shipment->awb }}</dd></div>@endif<div><dt>Total</dt><dd>Rs {{ number_format($order->total, 2) }}</dd></div></dl>
